...
\end{align*}
\missingfigure{max letzter teil}
...
Beispiel: $V=C[a,b]$
\begin{itemize}
  \item $\norm{f}_{L2(a,b)} = \sqrt{\int_a^b \abs{f(x)}^2 \mathrm{d}x}$ (L2-Norm)
...
\begin{align*}
  \inner{f}{g}_{L2(a,b)} := \int_a^b f(x) \overline{g(x)} \mathrm{d}x
\end{align*}
...
Beispiel: $L2(a,b)$
\begin{align*}
  \abs{\int_a^b f(x)\overline{g(x)}\mathrm{d}x} \leq \sqrt{\int_a^b \abs{f(x)}^2 \mathrm{d}x}\sqrt{\int_a^b \abs{g(x)}^2 \mathrm{d}x}
\end{align*}
...
\end{align*}
\missingfigure{Maximumnorm vs L2-Norm}
